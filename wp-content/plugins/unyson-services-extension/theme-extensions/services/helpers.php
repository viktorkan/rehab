...
function fw_ext_services_get_icon( $post_id = 0 ) {
	...
	return fw_get_db_post_option($post_id, 'service-icon', array());
}

function fw_ext_services_get_small_description( $post_id = 0 ) {
	if ( 0 === $post_id && null === ( $post_id = get_the_ID() ) ) {
		return array();
	}

	return fw_get_db_post_option( $post_id, 'small-description', array() );
}
 ...
 */
function fw_ext_services_get_listing_categories( $term_ids ) {
 ...
 * @return array
 */
function fw_ext_services_get_sort_classes( array $items, array $categories, $prefix = 'category_' ) {
