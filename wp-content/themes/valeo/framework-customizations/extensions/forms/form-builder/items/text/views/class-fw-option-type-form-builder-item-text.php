...
class FW_Option_Type_Form_Builder_Item_Text extends FW_Option_Type_Form_Builder_Item {
	...
	public function get_thumbnails() {
		return array(
			array(
				'html' =>
					'<div class="item-type-icon-title" data-hover-tip="' . esc_html__( 'Add a Single Line Text', 'valeo' ) . '">' .
					'<div class="item-type-icon"><img src="' . esc_attr( $this->get_uri( '/static/images/icon.png' ) ) . '" /></div>' .
					'<div class="item-type-title">' . esc_html__( 'Single Line Text', 'valeo' ) . '</div>' .
					'</div>'
			)
		);
	}
	...
	public function enqueue_static() {
		...
		wp_localize_script(
			...
			array(
				'l10n'     => array(
					'item_title'      => esc_html__( 'Single Line Text', 'valeo' ),
					...
					'options' => array(
						array(
							'label' => array(
								...
								'value' => esc_html__( 'Single Line Text', 'valeo' ),
							)
						),
						array(
							'required' => array(
	...
	public function frontend_render( array $item, $input_value ) {
		...
		return fw_render_view(
			$this->locate_path( '/views/view.php', get_template_directory() . '/framework-customization/extensions/forms/form-builder/items/text/view.php' ),
			array(
				'item' => $item,
				'attr' => $attr,
			)
