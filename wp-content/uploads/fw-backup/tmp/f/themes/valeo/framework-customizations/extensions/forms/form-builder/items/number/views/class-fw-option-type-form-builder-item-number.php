...
class FW_Option_Type_Form_Builder_Item_Number extends FW_Option_Type_Form_Builder_Item {
	...
	public function get_thumbnails() {
		return array(
			array(
				'html' =>
					'<div class="item-type-icon-title" data-hover-tip="' . esc_html__( 'Add a Number field', 'valeo' ) . '">' .
					'<div class="item-type-icon"><img src="' . esc_attr( $this->get_uri( '/static/images/icon.png' ) ) . '" /></div>' .
					'<div class="item-type-title">' . esc_html__( 'Number', 'valeo' ) . '</div>' .
					'</div>'
			)
		);
	}
	...
	public function enqueue_static() {
		...
		wp_localize_script(
			...
			array(
				...
				'l10n'     => array(
					'item_title'      => esc_html__( 'Number', 'valeo' ),
							...
							'constraints' => array(
								...
								'picker'  => array(
									'constraint' => array(
										...
										'desc'    => esc_html__( 'Set digits or values restrictions of this field', 'valeo' ),
										'type'    => 'radio',
										'inline'  => true,
										'choices' => array(
											'digits' => esc_html__( 'Digits', 'valeo' ),
											'value'  => esc_html__( 'Value', 'valeo' )
										),
									)
								),
								'choices' => array(
	...
	public function frontend_render( array $item, $input_value ) {
		...
		return fw_render_view(
			$this->locate_path( '/views/view.php', get_template_directory() . '/framework-customization/extensions/forms/form-builder/items/number/view.php' ),
			array(
				'item' => $item,
				'attr' => $attr,
			)
	...
	public function frontend_validate( array $item, $input_value ) {
		...
		$messages = array(
			'invalid'             => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( 'The {label} field must be a valid number', 'valeo' )
			),
			...
			'digits_min_singular' => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( 'The {label} field must have minimum %d digit', 'valeo' )
			),
			'digits_min_plural'   => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( 'The {label} field must have minimum %d digits', 'valeo' )
			),
			'digits_max_singular' => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( 'The {label} field must have maximum %d digit', 'valeo' )
			),
			'digits_max_plural'   => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( 'The {label} field must have maximum %d digits', 'valeo' )
			),
			'value_min'           => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( 'The {label} field minimum value must be %s', 'valeo' )
			),
			'value_max'           => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( 'The {label} field maximum value must be %s', 'valeo' )
			),
		);
