...
class FW_Option_Type_Form_Builder_Item_Select extends FW_Option_Type_Form_Builder_Item {
	...
	public function get_thumbnails() {
		return array(
			array(
				'html' =>
					'<div class="item-type-icon-title" data-hover-tip="' . __( 'Add a Dropdown', 'valeo' ) . '">' .
					'<div class="item-type-icon">' .
					'<img src="' . esc_attr( $this->get_uri( '/static/images/icon.png' ) ) . '" />' .
					'</div>' .
					'<div class="item-type-title">' . __( 'Dropdown', 'valeo' ) . '</div>' .
					'</div>'
			)
		);
	}
	...
	public function enqueue_static() {
		...
		wp_localize_script(
			...
			array(
				'l10n'     => array(
					'item_title'      => __( 'Dropdown', 'valeo' ),
					'label'           => __( 'Label', 'valeo' ),
					'toggle_required' => __( 'Toggle mandatory field', 'valeo' ),
					'edit'            => __( 'Edit', 'valeo' ),
					'delete'          => __( 'Delete', 'valeo' ),
					'edit_label'      => __( 'Edit Label', 'valeo' ),
				),
	...
	private function get_options() {
		return array(
					...
					'options' => array(
						array(
							'label' => array(
								'type'  => 'text',
								'label' => __( 'Label', 'valeo' ),
								'desc'  => __( 'Enter field label (it will be displayed on the web site)', 'valeo' ),
								'value' => __( 'Dropdown', 'valeo' ),
							)
						),
						array(
							'required' => array(
								'type'  => 'switch',
								'label' => __( 'Mandatory Field', 'valeo' ),
								'desc'  => __( 'Make this field mandatory?', 'valeo' ),
								'value' => true,
							)
						),
					)
				...
				'g2' => array(
					...
					'options' => array(
						array(
							'choices' => array(
								'type'   => 'addable-option',
								'label'  => __( 'Choices', 'valeo' ),
								'desc'   => __( 'Add choice', 'valeo' ),
								'option' => array(
									'type' => 'text',
								),
							)
						...
						array(
							'randomize' => array(
								'type'  => 'switch',
								'label' => __( 'Randomize', 'valeo' ),
								'desc'  => __( 'Do you want choices to be displayed in random order?', 'valeo' ),
								'value' => false,
							)
						),
					)
			...
			array(
				'info' => array(
					'type'  => 'textarea',
					'label' => __( 'Instructions for Users', 'valeo' ),
					'desc'  => __( 'The users will see these instructions in the tooltip near the field', 'valeo' ),
				)
			),
	...
	protected function get_fixed_attributes( $attributes ) {
		...
		if ( empty( $attributes['options']['choices'] ) ) {
			$attributes['options']['choices'][] = __( 'Dropdown', 'valeo' );
		}
	...
	public function frontend_validate( array $item, $input_value ) {
		...
		$messages = array(
			...
			'not_existing_choice' => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( '{label}: Submitted data contains not existing choice', 'valeo' )
			),
		);
