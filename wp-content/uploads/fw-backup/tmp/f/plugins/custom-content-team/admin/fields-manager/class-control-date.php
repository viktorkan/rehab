 ...
 * @access public
 */
class CCT_Fields_Control_Date extends CCT_Fields_Control {
	...
	public function content_template( $post_id ) {
		...
		$year_field  = $this->get_year_field( "cct_{$this->manager->name}_setting_{$this->setting}_year",   $year  );
		$month_field = $this->get_month_field( "cct_{$this->manager->name}_setting_{$this->setting}_month", $month );
		$day_field   = $this->get_day_field( "cct_{$this->manager->name}_setting_{$this->setting}_day",     $day   ); ?>

		<label>
			<?php if ( $this->label ) : ?>
				<span class="cct-fields-label"><?php echo esc_html( $this->label ); ?></span>
				<br />
			<?php endif; ?>

			<?php // Translators: 1: month, 2: day, 3: year.
			printf( __( '%1$s %2$s, %3$s', 'custom-content-team' ), $month_field, $day_field, $year_field ); ?>

			<?php if ( $this->description ) : ?>
				<br />
				<span class="cct-fields-description description"><?php echo $this->description; ?></span>
			<?php endif; ?>
		</label>
	<?php }
	...
	public function get_year_field( $name, $value ) {
		...
		return sprintf(
			...
			esc_html__( 'Year', 'custom-content-team' ),
			esc_attr( $name ),
			esc_attr( $value ),
			esc_attr( date( 'Y' ) )
		);
	...
	public function get_month_field( $name, $value ) {
		...
		for ( $i = 1; $i < 13; $i = $i +1 ) {
			...
			$options .= sprintf(
				...
				sprintf( esc_html__( '%1$s-%2$s', 'custom-content-team' ), $monthnum, $monthtext )
			);
		}

		return sprintf(
			'<label><span class="screen-reader-text">%s</span><select name="%s">%s</select></label>',
			esc_html__( 'Month', 'custom-content-team' ),
			esc_attr( $name ),
			$options
		);
	}
	...
	public function get_day_field( $name, $value ) {
		...
		return sprintf(
			...
			esc_html__( 'Day', 'custom-content-team' ),
			esc_attr( $name ),
			esc_attr( $value ),
			esc_attr( date( 'd' ) )
		);
