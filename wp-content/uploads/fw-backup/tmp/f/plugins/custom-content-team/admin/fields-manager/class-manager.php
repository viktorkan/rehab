...
class CCT_Fields_Manager {
	...
	private function register() {
		...
		do_action( "cct_{$this->name}_manager_register", $this );
	}
	...
	public function register_section( $section, $args = array() ) {
		...
		if ( ! is_object( $section ) )
			$section = new CCT_Fields_Section( $this, $section, $args );
	...
	public function register_control( $control, $args = array() ) {
		...
		if ( ! is_object( $control ) )
			$control = new CCT_Fields_Control( $this, $control, $args );
	...
	public function register_setting( $setting, $args = array() ) {
		...
		if ( ! is_object( $setting ) )
			$setting = new CCT_Fields_Setting( $this, $setting, $args );
	...
	public function display( $post_id ) {
		...
		$id    = sanitize_html_class( "cct-fields-manager-{$this->name}" );
		$class = 'cct-fields-manager ' . sanitize_html_class( "cct-fields-manager-{$this->name}" ); ?>

		<div id="<?php echo $id; ?>" class="<?php echo $class; ?>">
			<?php $this->nav( $post_id ); ?>
			<?php $this->content( $post_id ); ?>
		</div><!-- .cct-fields-manager -->
	<?php }
	...
	public function nav() { ?>
		...
		<?php foreach ( $this->sections as $section ) : ?>
			...
			<li>
				<a href="<?php echo esc_attr( "#cct-fields-section-{$section->name}" ); ?>"><i class="<?php echo $icon; ?>"></i> <span class="label"><?php echo esc_html( $section->label ); ?></span></a>
			</li>
	...
	public function content( $post_id ) {
		...
		wp_nonce_field( "cct_{$this->name}_nonce", "cct_{$this->name}" ); ?>

		<div class="cct-fields-content">

		<?php foreach ( $this->sections as $section ) : ?>

			<div id="<?php echo esc_attr( "cct-fields-section-{$section->name}" ); ?>" class="cct-fields-section <?php echo esc_attr( "cct-fields-section-{$section->name}" ); ?>">

				<?php if ( $section->description ) : ?>
					<p class="cct-fields-description description"><?php echo $section->description; ?></p>
				<?php endif; ?>
				...
				<?php foreach ( $this->controls as $control ) : ?>
					...
					<?php if ( $section->name === $control->section ) : ?>
						...
						<div id="<?php echo esc_attr( "cct-fields-control-{$control->name}" ); ?>" class="cct-fields-control <?php echo esc_attr( "cct-fields-control-type-{$control->type}" ); ?>">
							<?php $control->content_template( $post_id ); ?>
						</div><!-- .cct-fields-control -->

					<?php endif; ?>

				<?php endforeach; ?>

			</div><!-- .cct-fields-section -->

		<?php endforeach; ?>

		</div><!-- .cct-fields-content -->
	<?php }
	...
	public function update( $post_id ) {
		...
		// Verify the nonce.
		if ( ! isset( $_POST["cct_{$this->name}"] ) || ! wp_verify_nonce( $_POST["cct_{$this->name}"], "cct_{$this->name}_nonce" ) )
			return;
