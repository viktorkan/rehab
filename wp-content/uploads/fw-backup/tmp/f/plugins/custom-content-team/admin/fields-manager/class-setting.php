...
class CCT_Fields_Setting {
	...
	public function __construct( $manager, $name, $args = array() ) {
		...
		if ( $this->sanitize_callback )
			add_filter( "cct_{$this->manager->name}_sanitize_{$this->name}", $this->sanitize_callback, 10, 2 );
	}
	...
	public function get_value( $post_id ) {
		...
		return cct_get_member_meta( $post_id, $this->name );
	}
	...
	public function get_posted_value() {
		...
		if ( isset( $_POST[ "cct_{$this->manager->name}_setting_{$this->name}" ] ) )
			$value = $_POST[ "cct_{$this->manager->name}_setting_{$this->name}" ];
	...
	public function sanitize( $value ) {
		...
		return apply_filters( "cct_{$this->manager->name}_sanitize_{$this->name}", $value, $this );
	}
	...
	public function save( $post_id ) {
		...
		if ( '' == $new_value && $old_value )
			cct_delete_member_meta( $post_id, $this->name );

		// If the new value doesn't match the old value, set it.
		else if ( $new_value !== $old_value )
			cct_set_member_meta( $post_id, $this->name, $new_value );
	}
}
